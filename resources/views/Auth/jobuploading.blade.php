...
<div class="container mx-auto px-4 py-6">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden xl:table-cell">Experience</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">Deadline</th>
    ...
    <!-- Pagination -->
    <div class="max-w-full px-4 py-4 flex flex-col sm:flex-row items-center justify-between text-sm text-gray-700 bg-white rounded-b-lg shadow-sm border border-gray-200">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
            <div class="flex space-x-1 w-full sm:w-auto overflow-x-auto whitespace-nowrap p-2 sm:p-0 justify-center sm:justify-start">
                @if($jobs->onFirstPage())
                    <button class="px-3 py-1 rounded-lg border border-gray-300 bg-gray-100 text-gray-400 cursor-not-allowed">Previous</button>
                @else
                    <a href="{{ $jobs->previousPageUrl() }}" class="px-3 py-1 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors">Previous</a>
                @endif
                ...
                @if($current > 1 + $window)
                    <a href="{{ $jobs->url(1) }}" class="px-3 py-1 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors">1</a>
                @endif
                @if($current > 2 + $window)
                ...
                @for($page = max(1, $current - $window); $page <= min($last, $current + $window); $page++)
                    ...
                    @else
                        <a href="{{ $jobs->url($page) }}" class="px-3 py-1 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors">{{ $page }}</a>
                    @endif
                @endfor
                ...
                @if($current < $last - $window && $last > 1)
                    <a href="{{ $jobs->url($last) }}" class="px-3 py-1 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors">{{ $last }}</a>
                @endif

                @if($jobs->hasMorePages())
                    <a href="{{ $jobs->nextPageUrl() }}" class="px-3 py-1 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors">Next</a>
                @else
...
<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden" id="jobModal">
    <div class="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form action="{{ route('jobs.store') }}" method="POST" class="mt-4 space-y-4">
                @csrf
...
function openDeleteJobModal(id) {
    ...
    form.action = `/jobs/${id}`; // dynamically set action route
